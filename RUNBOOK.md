...
- **Livros em português (ou es/fr/hi/it) processados antes da OS-034 têm frases cortadas** — até a OS-034, qualquer texto cuja fonemização passasse de 510 fonemas era truncado silenciosamente pelo Kokoro nesses idiomas (só um `logger.warning`), com o áudio terminando no meio da palavra. Português rende ~1,19 fonemas por caractere, então frases acima de ~430 caracteres sem `.!?` interno eram afetadas — medido: uma frase de 557 caracteres perdia 22% do conteúdo. **Inglês nunca foi afetado** (o `en_tokenize` do Kokoro já dividia respeitando o limite). Não há reprocessamento automático — reenviar manualmente os livros nesses idiomas que já estavam `ready` antes desta correção. Detalhes em `docs/report/OS-034-report.md`.
