...
**Fase:** OS-021 concluída — `core/pipeline.py::synthesize_text()` ganhou parâmetro opcional `on_chunk`; `worker/tasks.py` persiste cada `AudioChunk` incrementalmente via callback (não mais uma chamada só no final do livro inteiro) e marca `Book.status = "synthesizing"` antes de começar o loop. `GET /books/{id}/audio` (que já não verificava status) agora retorna chunks parciais durante a síntese. Ver `docs/report/OS-021-report.md`.

**Última OS concluída:** OS-021 — entrega incremental de áudio. PR a abrir.

**OS em andamento:** nenhuma.

**Próxima OS a abrir:** OS-022 (retomar processamento interrompido, depende da OS-021 — ver `docs/os/OS-022-retomar-processamento.md`): hoje um `Job` interrompido no meio fica órfão em `status="running"` pra sempre, perde todo o trabalho já feito. Com chunks já persistidos incrementalmente (OS-021), o resume pode pular o que já foi sintetizado.
...
| `core/pipeline.py` | concluído (testado) | OS-021 | `extract_with_fallback()`, `extract_clean_text()` e `synthesize_text(text, chapter_id, max_chars=None, on_chunk=None)` — if `on_chunk` is passed, called with each chunk immediately after synthesis; returns full list unchanged (backward-compatible) |
...
| `worker/tasks.py` | concluído (testado) | OS-021 | `process_job(job)` persiste cada `AudioChunk` incrementalmente via callback `on_chunk` (não mais uma chamada só no final), marca `Book.status = "synthesizing"` antes de começar a síntese; `run_worker(poll_interval, max_iterations)` faz polling; `python -m worker.tasks` para rodar manualmente |
...
20. **OS-020 — Detecção automática de idioma pro Kokoro** — bug real encontrado em uso (PDF em português sintetizado em inglês) — status: aberta, aguardando execução (ver `docs/os/OS-020-deteccao-idioma-kokoro.md`)
...
22. **OS-022 — Retomar processamento interrompido** (depende da OS-021) — hoje um `Job` interrompido no meio fica órfão pra sempre, tem que recomeçar do zero — status: aberta, aguardando execução (ver `docs/os/OS-022-retomar-processamento.md`)
23. Reenviar livros já processados antes da OS-019 (pronúncia incorreta) — inclusive o "Security Engineering" que originou a investigação
24. Revisitar o threshold de confiança `0.85` especificamente para `EasyOCRExtractor`, à luz do achado da OS-017 — recomendado antes do item 25
25. Ligar `EasyOCRExtractor` na cadeia de fallback de `core/pipeline.py`
